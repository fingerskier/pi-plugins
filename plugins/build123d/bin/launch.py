"""Bootstrap launcher for the build123d MCP server in Pi.

Creates an isolated venv under ${PI_PLUGIN_DATA}/venv (or the Claude-compatible
${CLAUDE_PLUGIN_DATA}) on first run, installs this package into it, then execs
`build123d-mcp`. All progress logs go to stderr so stdout remains MCP JSON-RPC.
"""
...
def log(msg: str) -> None:
    print(f"[build123d-pi launcher] {msg}", file=sys.stderr, flush=True)
...
def resolve_env_dirs() -> tuple[Path, Path]:
    root = os.environ.get("PI_PLUGIN_ROOT") or os.environ.get("CLAUDE_PLUGIN_ROOT")
    data = os.environ.get("PI_PLUGIN_DATA") or os.environ.get("CLAUDE_PLUGIN_DATA")
    if not root:
        die("PI_PLUGIN_ROOT is not set; launcher must be invoked by the Pi extension.")
    if not data:
        die("PI_PLUGIN_DATA is not set; launcher must be invoked by the Pi extension.")
...
def venv_entrypoint(venv_dir: Path) -> Path:
    if os.name == "nt":
        return venv_dir / "Scripts" / "build123d-mcp.exe"
    return venv_dir / "bin" / "build123d-mcp"
...
def compute_source_hash(root: Path) -> str:
    h = hashlib.sha256()
    for rel in ["pyproject.toml", "README.md", "README.upstream.md"]:
        path = root / rel
        if path.exists():
            h.update(rel.encode())
            h.update(b"\0")
...
def install_if_stale(root: Path, data: Path, venv_dir: Path) -> None:
    ...
    if current == previous and venv_entrypoint(venv_dir).exists():
        return
    log("installing build123d MCP package into venv (first run or package updated)")
    try:
