"""Bootstrap launcher for the SKiDL MCP server in Pi.
...
`skidl-mcp`. All progress logs go to stderr so stdout remains MCP JSON-RPC.
"""
...
def log(msg: str) -> None:
    print(f"[skidl-pi launcher] {msg}", file=sys.stderr, flush=True)
...
def venv_entrypoint(venv_dir: Path) -> Path:
    if os.name == "nt":
        return venv_dir / "Scripts" / "skidl-mcp.exe"
    return venv_dir / "bin" / "skidl-mcp"
...
def install_if_stale(root: Path, data: Path, venv_dir: Path) -> None:
    ...
    if current == previous and venv_entrypoint(venv_dir).exists():
        return
    log("installing SKiDL MCP package into venv (first run or package updated)")
    try:
